...
ControlCommand Se3Control::runControl(const DesiredState &des,
  ...
  const auto odom_quat =
      Eigen::Quaterniond(odom_q.w, odom_q.x, odom_q.y, odom_q.z);
...
void Se3Control::resetThrustMapping() {
  ...
  P = 1e6;
  spdlog::info("Reset thrust mapping(Linear Model), hover thrust from {} to {}",
               quad_params_.g / thr2acc, quad_params_.g / init_thr2acc);
