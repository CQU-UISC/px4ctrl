...
Px4Ctrl::Px4Ctrl(std::shared_ptr<Px4CtrlRosBridge> px4_bridge,
    ...
    : px4_bridge(px4_bridge), px4_state(px4_state), px4_server(px4_server),
      px4ctrl_params(px4ctrl_params) {
  assert(px4_bridge != nullptr && px4_state != nullptr &&
         px4ctrl_params != nullptr);
  if (!init()) {
    spdlog::error("Px4Ctrl init failed");
    throw std::runtime_error("Px4Ctrl init failed");
  }
  controller = std::make_shared<controller::Se3Control>(
      px4ctrl_params->control_params, px4ctrl_params->quadrotor_params);
}
...
void Px4Ctrl::client_command_callback(const ui::ClientPayload &payload) {
  ...
  case ui::ClientCommand::TAKEOFF:
    if (!(px4ctrl_fsm_state[0] == Px4CtrlState::L0_OFFBOARD &&
          px4ctrl_fsm_state[1] == Px4CtrlState::L1_ARMED)) {
      spdlog::error("Reject! beacuse L0!=OFFBOARD or L1!=ARMED, L0:{},L1:{}",
  ...
  case ui::ClientCommand::CHANGE_HOVER_POS:
    ...
    if (px4ctrl_fsm_state[2] != Px4CtrlState::L2_HOVERING) {
      spdlog::error("Reject! beacuse L2!=HOVERING, L2:{}",
                    state_map(px4ctrl_fsm_state[2]));
      break;
    }
    double data[7];
...
*/
  ...
  if (timePassed(last_log_state_time) > 1000) {
    last_log_state_time = clock::now();
    spdlog::debug("L0:{},L1:{},L2:{}", state_map(L0.state), state_map(L1.state),
                  state_map(L2.state));
  }
}
...
void Px4Ctrl::process_l0(controller::ControlCommand &ctrl_cmd) {
  ...
  // update state
  if (px4_state->state->value().first->mode ==
  ...
  if (L0.next_state != L0.state) {
    spdlog::info("L0 state change:{}->{}", state_map(L0.state),
...
void Px4Ctrl::process_l2(controller::ControlCommand &ctrl_cmd) {
  ...
  case L2_IDLE: { // default
    ...
    if (timePassed(L2idle.last_arm_time) >
        px4ctrl_params->statemachine_params.l2_idle_disarm_time) {
  ...
  case L2_TAKING_OFF: {
    if (L2.last_state != L2_TAKING_OFF) {
      ...
      spdlog::info("Taking off to:{} {} {}", L2takingoff.start_pos.x(),
                   ...
                   L2takingoff.start_pos.z() +
                       px4ctrl_params->statemachine_params.l2_takeoff_height);
    } else {
    ...
    des.v = Eigen::Vector3d(
        0, 0, px4ctrl_params->statemachine_params.l2_takeoff_landing_speed);
    des.yaw = controller::yawFromQuat(des.q);
    des.p.z() += px4ctrl_params->statemachine_params.l2_takeoff_landing_speed *
                 timePassedSeconds(L2takingoff.last_takeoff_time);
  ...
  case L2_HOVERING: {
    ...
    if (L2.last_state != L2_HOVERING) {
      ...
      spdlog::info("Hovering at->X:{} Y:{} Z:{}", L2hovering.des_pos.x(),
                   L2hovering.des_pos.y(), L2hovering.des_pos.z());
    }
  ...
  case L2_LANDING: {
    ...
    const double POSITION_DEVIATION_C =
        px4ctrl_params->statemachine_params
            .l2_land_position_deviation_c; // Constraint 1: target position
                                           // below real position for
                                           // POSITION_DEVIATION_C meters.
    const double VELOCITY_THR_C =
        px4ctrl_params->statemachine_params
            .l2_land_velocity_thr_c; // Constraint 2: velocity below
                                     // VELOCITY_MIN_C m/s.
    const double TIME_KEEP_C =
        px4ctrl_params->statemachine_params
            .l2_land_time_keep_c; // Constraint 3: Time(ms) the Constraint 1&2
                                  // need to keep.

    bool C12_satisfy =
        (des.p(2) - px4_state->odom->value().first->pose.pose.position.z) <
            POSITION_DEVIATION_C &&
    ...
    if (C12_satisfy) {
      ...
      if (timePassed(L2landing.time_C12_reached) >
          TIME_KEEP_C) { // Constraint 3 reached
        spdlog::info("Successfully landed");
        landed = true;
      }
    }
    spdlog::debug("Land Des Position:x :{} y:{} z:{}, velocity: v:{}, "
                  "ctrl_cmd: thrust:{}\n",
                  des.p.x(), des.p.y(), des.p.z(), des.v.z(), ctrl_cmd.thrust);
    spdlog::debug("Landing: C12_satisfy:{}, landed:{}, Landed_state:{}\r",
                  C12_satisfy, landed,
                  px4_state->ext_state->value().first->landed_state);
  ...
  case L2_ALLOW_CMD_CTRL: {
    ...
    if (cmdctrl_hz >= px4ctrl_params->statemachine_params.l2_cmd_ctrl_min_hz) {
      ...
      spdlog::error("cmdctrl_hz:{} is less than min_hz:{}", cmdctrl_hz,
  ...
  case L2_CMD_CTRL: {
    ...
    case px4ctrl_msgs::msg::Command::THRUST_BODYRATE: {
      ...
      ctrl_cmd.bodyrates =
          Eigen::Vector3d(px4_state->ctrl_command->value().first->u[1],
    ...
    case px4ctrl_msgs::msg::Command::THRUST_QUAT: {
      ...
      ctrl_cmd.thrust =
          controller->thrustMap(px4_state->ctrl_command->value().first->u[0]);
      ctrl_cmd.attitude = Eigen::Quaterniond(
          des_quat[0], des_quat[1], des_quat[2], des_quat[3]); // w,x,y,z
      break;
    }
    }
    break;
