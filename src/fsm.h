...
/*
    enum Px4CtrlState{
        ...
        TAKING_OFF,//  (手动进入)
        HOVERING ,//
   (TAKING_OFF后自动进入，或者CMD_CTRL命令超时后进入，或者是地面站强制进入,
   或者是Guard触发) ALLOW_CMD_CTRL,//  (在HOVERING下用户手动进入) CMD_CTRL,//
   （在ALLOW_CMD_CTRL收到命令后自动进入） LANDING ,//
   (UNARM，在HOVERING下由用户手动/Guard触发，执行成功后进入UNARMED)
    };
*/
struct LState {
