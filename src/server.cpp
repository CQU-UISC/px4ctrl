...
ZmqProxy::ZmqProxy(zmq::context_t &ctx, const ZmqParas &paras)
    : paras(paras), xsub_endpoint(paras.xsub_bind),
      xpub_endpoint(paras.xpub_bind), xsub_socket(ctx, zmq::socket_type::xsub),
...
ZmqProxy::~ZmqProxy() { join(); }

Px4Server::Px4Server(zmq::context_t &ctx, const ZmqParas &paras)
    : paras(paras), server_pub(ctx, zmq::socket_type::pub),
  ...
  while (ok) {
    try {
      ...
      zmq::recv_result_t result =
          zmq::recv_multipart(client_sub, std::back_inserter(msgs));
      if (!result) {
